...
def get_power_mode(display: int | None = None, bus: int | None = None) -> int:
    v = ddcutil.get_vcp(POWER_MODE_CODE, display=display, bus=bus)
    return v.current


def power_on(display: int | None = None, bus: int | None = None) -> None:
    ddcutil.set_vcp(POWER_MODE_CODE, POWER_ON, display=display, bus=bus)


def power_off(display: int | None = None, bus: int | None = None) -> None:
    ddcutil.set_vcp(POWER_MODE_CODE, POWER_OFF, display=display, bus=bus)


def toggle_power(display: int | None = None, bus: int | None = None) -> str:
